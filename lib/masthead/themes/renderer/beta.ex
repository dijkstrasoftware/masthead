defmodule Masthead.Themes.Renderer.Beta do
  @moduledoc """
  The original render contract, frozen.

  Themes whose manifest names no `render_version` render through this module
  forever. It exposes per-page options under `page.metadata` and knows nothing
  about post options — a post's raw `post_options` map rides along in the
  context unread, because no theme written against this contract can reference
  it.
